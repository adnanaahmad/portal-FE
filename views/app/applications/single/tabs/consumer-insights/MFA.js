...
import { cancelMFA, checkMFA, runMFA } from "../../../../../../utils/Thunk";
...
class MFA extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      const { mfa } = application;
      const { mfa: mfaPrev } = applicationPrev;
  ...
  checkMFA() {
    ...
    this.props.dispatch(
      checkMFA(application.id, null, (res) => {
        if (res && res.success) {
          if (res.application && onRefresh) onRefresh(res.application);
  ...
  renderForm() {
    ...
    const { phone, checked } = this.state;
    const { mfa } = application;
      ...
      <div className={styles[0].consumerInsightsSectionMFA__body}>
        {content && (
          <div className={["mt-3", styles[0].verificationItemsGroup].join(" ")}>
            <VerificationItem title="Twilio Report" content={content} />
          </div>
        )}
        {this.renderForm()}
      </div>
